-- Esquema de Base de Datos para Hermes Control
...
CREATE TABLE jornadas (
    ...
    id_parte INTEGER REFERENCES partes_trabajo(id) ON DELETE SET NULL
);
...
-- 7. Tabla: partes_trabajo (Asignaciones para conductores)
CREATE TABLE partes_trabajo (
    id SERIAL PRIMARY KEY,
    id_conductor INTEGER NOT NULL REFERENCES usuarios(id) ON DELETE RESTRICT,
    fecha_hora_recogida TIMESTAMP WITH TIME ZONE NOT NULL,
...
CREATE INDEX idx_partes_conductor ON partes_trabajo(id_conductor);
-- Esquema de Base de Datos para Hermes Control

-- Eliminar tablas si existen (para facilitar reinicios)
DROP TABLE IF EXISTS historial_partes CASCADE;
DROP TABLE IF EXISTS partes_trabajo CASCADE;
DROP TABLE IF EXISTS limpiezas CASCADE;
DROP TABLE IF EXISTS repostajes CASCADE;
DROP TABLE IF EXISTS pausas_jornada CASCADE;
DROP TABLE IF EXISTS jornadas CASCADE;
DROP TABLE IF EXISTS vehiculos CASCADE;
DROP TABLE IF EXISTS usuarios CASCADE;
