...
export async function POST(req) {
  try {
    ...
    if (active.rows.length === 0) {
      return NextResponse.json({ error: 'No tienes una jornada activa (o está pausada) para registrar limpiezas.' }, { status: 400 });
    }

    const id_jornada = active.rows[0].id;

    const result = await pool.query(
      'INSERT INTO limpiezas (id_jornada, cantidad_euros) VALUES ($1, $2) RETURNING *',
