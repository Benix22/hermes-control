...
export async function GET(req) {
  try {
    ...
    let sql = `
      ...
      FROM limpiezas l 
      JOIN jornadas j ON l.id_jornada = j.id 
    ...
    if (fechaInicio) {
      ...
      sql += ` AND l.fecha_hora >= $${params.length}`;
    }
    if (fechaFin) {
    ...
    return NextResponse.json({ resumen: { totalEuros, totalLimpiezas: rows.length }, detalles: rows });
  } catch (err) {
    console.error(err);
